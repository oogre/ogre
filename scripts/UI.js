(function(){
	...
	OGRE.UI = function(){
		...
		var _createPictureViewer = function(gallery, images){
			...
			gallery.parentNode.addEventListener('scroll', function(event){
				var position = this.scrollLeft / $(this).width();
				var destination = (gallery.oldscrollLeft - this.scrollLeft<0 ? Math.ceil(position) : Math.floor(position)) * $(this).width();
				gallery.oldscrollLeft = this.scrollLeft;
				clearTimeout(gallery.timer);
				gallery.timer = setTimeout(function(){
					_scrollTo(gallery.parentNode, destination);
				}, 75);
			}, false);
			...
			gallery.parentNode.addEventListener('mousemove', function(event){
				...
				if(x < 33){
					...
					false === this.classList.contains('fullscreen') && setfullscreen.classList.remove('hide');
				}
				else{
					setfullscreen.classList.add('hide');
			...
			gallery.parentNode.addEventListener('mouseover', function(event) {
				
			}, false);

			gallery.parentNode.addEventListener('mouseleave', function(event) {
				//setswitch.classList.add('hide');
				setfullscreen.classList.add('hide');
			}, false);
